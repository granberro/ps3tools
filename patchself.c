...
{
	if (key_get(app_type, suffix, &ks) < 0)
		fail("key_get failed");
...
static void write_elf(void)
{
	...
	if (patched) {
		...
		u8 npdrm_omac_key1[0x10];
		u8 npdrm_omac_key2[0x10];

		// key version
		wbe16(self + 0x08, 01);

		// SCE Version Info file
		wbe32(self + ver_offset + 0x00, 0x01);
		wbe32(self + ver_offset + 0x04, 0x00);
		wbe32(self + ver_offset + 0x08, 0x10);
		wbe32(self + ver_offset + 0x0c, 0x00);

		wbe32(self + ver_offset + 0x10, 0x00);
		wbe32(self + ver_offset + 0x14, 0x00);
		wbe32(self + ver_offset + 0x18, 0x00);
		wbe32(self + ver_offset + 0x1c, 0x00);

		wbe32(self + ver_offset + 0x20, 0x00);
		wbe32(self + ver_offset + 0x24, 0x00);
		wbe32(self + ver_offset + 0x28, 0x00);
		wbe32(self + ver_offset + 0x2c, 0x00);

		// Control 
		memset(self + control_offset + 0x10, 0, 0x10);
		wbe16(self + control_offset + 0x10, 0x4000);
		wbe16(self + control_offset + 0x2E, 0x02);
		memset(self + control_offset + 0x54, 0, 0x14);
		wbe32(self + control_offset + 0x6C, 0x5DC0);
		wbe32(self + control_offset + 0x8c, 0x01);

		key_get_simple("npdrm_omac_key1", npdrm_omac_key1, sizeof npdrm_omac_key1);
		key_get_simple("npdrm_omac_key2", npdrm_omac_key2, sizeof npdrm_omac_key2);
		for(j=0;j<0x10;j++) npdrm_omac_key[j] = npdrm_omac_key1[j] ^ npdrm_omac_key2[j];

		memset(self + control_offset + 0xC0, 0, 0x10);

		aesOmac1Mode(self + control_offset + 0xE0, self + control_offset + 0x80, 0x60, npdrm_omac_key, sizeof(npdrm_omac_key)*8);
...
printf("\n");
		...
		sce_encrypt_npdrm(self, klist, klist->free_klicensee);
		//TODO
		//sce_add_npdrm(self, klist) < 0)
		//		        fail("self_add_npdrm failed");
		//TODO
...
int main(int argc, char *argv[])
{
	...
	if (argc < 3 || argc > 5)
		fail("usage: patchself in.self out.self");
